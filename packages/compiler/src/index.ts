...
function elementMarkup(
  ...
  for (const attribute of node.attrs) {
    ...
    const value = dynamicAttribute(attribute.value, filename, locals);
    if (name === 'bind:attrs') {
      if (!value) fail(filename, 'bind:attrs needs a record expression.');
...
export function compileComponentParts(
  ...
  code.append(
    '// Generated by workstar-compiler. Edit the source component instead.\n',
  );
  code.append(
    "import { html as __html, attr as __attr, attrs as __attrs, on as __on, repeat as __repeat, textareaValue as __textareaValue } from 'workstar';\n",
  );
  if (options.hotState) {
    code.append(
