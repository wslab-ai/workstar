...
This is experimental compatibility, not complete React semantics. Rendering is client-side; component state updates currently redraw the Workstar root and restore form focus, so DOM identity, selection in all input types, transition continuity, and large-page update performance are not yet equivalent to React. Events are native DOM events rather than SyntheticEvents. Only a restricted SVG data image is accepted for an `<img>` source. Other React and React Router APIs, portals, error boundaries, and general third-party React libraries still need compatibility work and browser verification.
